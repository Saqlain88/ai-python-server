...
@image_bp.route('/templates', methods=['POST'])
@admin_required
def create_template():
    try:
...
@image_bp.route('/templates/<template_id>', methods=['GET'])
def get_template(template_id):
    template = Template.query.get_or_404(template_id)
    return jsonify(template.to_dict()), 200

@image_bp.route('/templates/<template_id>', methods=['DELETE'])
@admin_required
...
@image_bp.route('/templates/<template_id>', methods=['PUT'])
@admin_required
    ...
    if 'image' in request.files:
        ...
        if file.filename != '':
            try:
                ...
                # Upload new image
                upload_result = image_service.upload_image(file)
                template.thumbnail = upload_result['secure_url']
